...
public class BrushableEntitiesResourceLoader implements SimpleSynchronousResourceReloadListener {
    ...
    public void reload(ResourceManager manager) {
        ...
        var brushable = manager.findResources("brushable/entity", identifier -> identifier.getPath()
                .endsWith(".json"));
        ...
        for (Map.Entry<Identifier, Resource> resourceEntry : brushable.entrySet()) {
            ...
            try (var input = resource.getInputStream()) {
                ...
                var entity = get(data,
                        resource.getPackId(),
                        "id",
                        jsonElement -> stringToEntityType(jsonElement.getAsString()),
                        jsonElement -> String.format("could not find entity '%s'", jsonElement.getAsString()));

                var particleBlock = get(data,
                        resource.getPackId(),
                        "particle_block",
                        jsonElement -> stringToBlock(jsonElement.getAsString()),
                        jsonElement -> String.format("could not find block '%s'", jsonElement.getAsString()));

                var minParticleCount = get(data,
                        resource.getPackId(),
                        "min_particle_count",
                        JsonElement::getAsInt,
                        jsonElement -> String.format("could not parse min_particle_count '%s'",
                                jsonElement.getAsString()));
                var maxParticleCount = get(data,
                        resource.getPackId(),
                        "max_particle_count",
                        JsonElement::getAsInt,
                        jsonElement -> String.format("could not parse max_particle_count '%s'",
                                jsonElement.getAsString()));
                ...
                var height = get(data,
                        ...
                        jsonElement -> String.format("could not parse height '%s'", jsonElement.getAsString()));
                var babyHeight = get(data,
                        resource.getPackId(),
                        "baby_height",
                        JsonElement::getAsFloat,
                        jsonElement -> String.format("could not parse baby_height '%s'", jsonElement.getAsString()));

                var lootTable = get(data,
                        resource.getPackId(),
                        "loot_table",
                        jsonElement -> Identifier.tryParse(jsonElement.getAsString()),
                        jsonElement -> String.format("could not parse loot_table '%s'", jsonElement.getAsString()));

                var entry = new BrushableEntityEntry(new BlockStateParticleEffect(ParticleTypes.BLOCK,
                        particleBlock.getDefaultState()),
                        minParticleCount,
                        maxParticleCount,
                        height,
                        babyHeight,
                ...
                if (previous != null) {
                    UsefulBrush.LOGGER.warn("%s -> %s got overwritten with %s".formatted(entity,
                            previous.toString(),
                            entry.toString()));
                } else {
                    count++;
